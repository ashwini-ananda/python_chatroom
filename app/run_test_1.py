...
def test_scenario_2():
  ...
  try:
    ...
    threading.Thread(target=alice.main, args=([['1',_server_ip, _port, 'Alice'],_clients,alice_output]), daemon=True).join()
    threading.Thread(target=bob.main, args=([['2',_server_ip, _port, 'Bob'],_clients,bob_output]), daemon=True).join()
    threading.Thread(target=chad.main, args=([['3',_server_ip, _port, 'Chad'],_clients,chad_output]), daemon=True).join()
  ...
  # reaed all outputs
    ...
    threading.Thread(target=bob.main, args=([['2',_server_ip, _port, 'Bob'],_clients,bob_output]), daemon=True).do_stop = True
    threading.Thread(target=chad.main, args=([['3',_server_ip, _port, 'Chad'],_clients,chad_output]), daemon=True).do_stop = True
    threading.Thread(target=doug.main, args=([['4',_server_ip, _port, 'Doug'],_clients,doug_output]), daemon=True).do_stop = True

    threading.Thread(target=alice.main, args=([['1',_server_ip, _port, 'Alice'],_clients,alice_output]), daemon=True).join()
    threading.Thread(target=bob.main, args=([['2',_server_ip, _port, 'Bob'],_clients,bob_output]), daemon=True).join()
    threading.Thread(target=chad.main, args=([['3',_server_ip, _port, 'Chad'],_clients,chad_output]), daemon=True).join()
    threading.Thread(target=doug.main, args=([['4',_server_ip, _port, 'Doug'],_clients,doug_output]), daemon=True).join()
  return
